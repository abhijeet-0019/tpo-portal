...
const getFileUploadParams = (title,filename_disk,filename_download,folder) => {
    return {
        title:title,
...
async function createItem(collectionName,payload,authorized=true,staticAccessToken=null){
...
async function updateItem (collectionName, id, payload,authorized=true,staticAccessToken=null) {
    const authHeader = getAuthHeader(staticAccessToken);
    if (!!authorized && !!authHeader){
        const directus = getDirectus(authHeader);
        const collection = directus.items(collectionName);
...
async function getItems(collectionName, fieldParam, limitParam, pageParam, filterParam, sortParam, searchParam,authorized=false){
    const queryObj= {
...
async function getItem(collectionName, itemID, fieldParam, authorized=false){
    const queryObj= {
        ...(fieldParam!==undefined ? { fields: fieldParam } : {})
    };
    const authHeader = getAuthHeader();
    let directus;
    if (!!authorized && !!authHeader)
        directus = getDirectus(authHeader);
    else
    ...
    if(directus){
        ...
        if(Array.isArray(itemID)){
            ...
            return await collection.readOne(itemID,queryObj);
    }
