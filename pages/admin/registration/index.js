 ...
 // const [file, setFile]= useState(null);
  const handleFormSubmit_personal = (event) => {
    ...
    if (!!formData_personal) {
      updateItem("TPO_students_personal_details", 3, formData_personal, true)
        .then((success) => {
    ...
    console.log(formData_personal.data);
  };
  const handleFormSubmit_academic = (event) => {
    ...
    if (!!formData_academic) {
      ...
      for(const key in formData_academic){
        formData.append(key, formData_academic[key]);
      }
      if(file){
        formData.append('marksheets', file);
      }  
*/
      updateItem("TPO_academic_details", 3, formData_academic, true)
        .then((success) => {
          alert("submission succesful");
        })
        .catch((fail) => {
        ...
        <Grid item xs={12}>
          <Button
            ...
            onClick={function (event) {
              handleFormSubmit_academic; handleFormSubmit_personal;
            }}
